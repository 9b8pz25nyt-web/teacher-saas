...
export async function generateReportFromScript(scriptText: string, studentName: string) {
  try {
    ...
    if (!apiKey) {
      return { error: "GEMINI_API_KEY is missing from Vercel environment variables." };
    }

    const ai = new GoogleGenAI({ apiKey });

    if (!scriptText || !scriptText.trim()) {
      return { error: "The uploaded script is empty." };
    }
...
"""`,
    ...
    if (!response) {
      return { error: "All AI model servers are currently busy. Please try again in a few seconds." };
    }

    const rawText = response.text || "{}";
    const jsonMatch = rawText.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      return { error: "AI did not return a valid JSON structure." };
    }

    return JSON.parse(jsonMatch[0]);
  } catch (err: any) {
    console.error("AI Server Action error:", err);
    return { error: err.message || "Failed to process AI script." };
  }
}